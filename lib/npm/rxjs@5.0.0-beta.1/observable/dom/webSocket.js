  ...
  WebSocketSubject.prototype._subscribe = function(subscriber) {
    ...
    if (self.url && !self.socket) {
      ...
      socket.onopen = function(e) {
        ...
        self.destination = Subscriber_1.Subscriber.create(function(x) {
          ...
          if (e && e.code) {
            ...
            self._finalError(new TypeError('WebSocketSubject.error must be called with an object with an error code, ' + 'and an optional reason: { code: number, reason: string }'));
          }
        }, function() {
          var closingObserver = self.closingObserver;
          if (closingObserver) {
